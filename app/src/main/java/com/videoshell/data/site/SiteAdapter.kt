...
abstract class SiteAdapter(val site: SiteConfig) {
     ...
     * 应该是校准没生效」。这句话背后至少有四种完全不同的原因：
     *
     * 1. 配方压根没写盘（走了「取消」把整场校准丢了）；
     * 2. 写盘了，但本站命中了加密白名单 ⇒ 校准规则被设计性地跳过；
     * 3. 写盘了也生效了，但**校准形状这一页只命中 1 个**（判据要求 ≥2）⇒ 静默退回默认逻辑；
     * 4. 生效了，用户看到的"没变"其实是"结果恰好一样"。
     *
     * 只靠"界面看着没变"永远分不出这四种 —— 所以让适配器把**它这一步真实的决策**
     * 报出来，`SiteDoctor` 原样打进报告。这一条是从「应该没生效」到「确定是哪一种」的桥。
     */
    open val calibDiag: String get() = ""

    /** 本适配器建完之后，配方是不是**被用上了**（自检用；不参与逻辑） */
    open val calibApplied: Boolean get() = false

    /**
     * ## 校准「当场自证」：这条分类形状在本页真能收到几个分类（v1.0.33）
     *
     * 必须复用**适配器运行时那一套收集器**（`HtmlAdapter.collectSlashDirCategories` /
     * `collectByCatTpl`）—— 判据只能有一份，否则就是「界面说学到了、实际解析不用」
     * 这种最难查的不一致。
     *
     * 为什么非要有它：用户在校准第一步点到的可能只是**一个导航项**。
     * 野果实测（2026-09-19）：`/explore/drama/` 是侧栏「探索分类」入口，
     * 在首页 330 个 `<a>` 里只出现 **1 次**，而真分类是 **252 条** `/tag/{slug}/`。
     * 但 [HtmlTemplates.catTplFrom] 会把它老老实实泛化成 `/explore/{slug}/`
     * —— 与真分类形状**一模一样**，形状永远分不开，**只有数量能分开**。
     *
     * 旧行为：照样写进配方 ⇒ 运行时命中 1 个（判据要求 ≥2）⇒ 静默退回默认逻辑
     * ⇒ 用户看到的就是「校准走完了，可用起来跟没校准一样」，而且永远查不出为什么。
     * 现在改成：**校准当场数一遍，不够 2 条就不收**，并且明确告诉他点错了什么。
     *
     * @return 命中数；不是 HTML 适配器 / 解析失败返回 **-1**（调用方据此放弃判断，别误杀）
     */
    open fun countCatTplHits(html: String, tpl: String): Int = -1
