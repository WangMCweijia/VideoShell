#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""跑 Extract.java：用 App 真实的 Media.extractFromHtml 解析真实播放页。"""
...
r = subprocess.run([JAVAC, '-encoding', 'UTF-8', '-nowarn', '-cp', cp, '-d', out,
                    os.path.join(HERE, 'Extract.java')],
                   capture_output=True, text=True, encoding='utf-8', errors='replace')
if r.returncode != 0:
    sys.stdout.write((r.stdout or '') + (r.stderr or ''))
    raise SystemExit('javac 失败')

r = subprocess.run([JAVA, '-Dfile.encoding=UTF-8', '-cp', out + os.pathsep + cp, 'Extract'],
                   capture_output=True, text=True, encoding='utf-8', errors='replace',
                   cwd=out)
txt = (r.stdout or '') + (('\n[stderr]\n' + r.stderr) if r.stderr else '')
sys.stdout.write(txt)
open(os.path.join(out, '_extract.out.txt'), 'w', encoding='utf-8').write(txt)
