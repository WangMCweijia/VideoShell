#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""跑 SiteNameTest.java：站名净化（E49）的离线回归（纯函数，无需网络/样本）。

真跑 `SiteDetector.cleanSiteName`，classpath 走 `_cp.classpath()`。
"""
...
r = subprocess.run([JAVA, '-Dfile.encoding=UTF-8', '-Dvs.root=' + ROOT,
                    '-cp', out + os.pathsep + cp, 'tools.verify.SiteNameTest'],
                   capture_output=True, text=True, encoding='utf-8', errors='replace')
txt = (r.stdout or '') + (('\n[stderr]\n' + r.stderr) if r.stderr else '')
sys.stdout.write(txt)
open(os.path.join(HERE, '_sn.out.txt'), 'w', encoding='utf-8').write(txt)
