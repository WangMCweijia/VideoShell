#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""跑 Extract2.java：验证「取最内层媒体地址」修复（合成用例 + 真实页面）。"""
...
out = os.path.join(HERE, '_extract2')
os.makedirs(out, exist_ok=True)

r = subprocess.run([JAVAC, '-encoding', 'UTF-8', '-nowarn', '-cp', cp, '-d', out,
                    os.path.join(HERE, 'Extract2.java')],
                   capture_output=True, text=True, encoding='utf-8', errors='replace')
if r.returncode != 0:
    sys.stdout.write((r.stdout or '') + (r.stderr or ''))
    raise SystemExit('javac 失败')

r = subprocess.run([JAVA, '-Dfile.encoding=UTF-8', '-cp', out + os.pathsep + cp, 'Extract2'],
                   capture_output=True, text=True, encoding='utf-8', errors='replace',
                   cwd=out)
txt = (r.stdout or '') + (('\n[stderr]\n' + r.stderr) if r.stderr else '')
sys.stdout.write(txt)
open(os.path.join(out, '_extract2.out.txt'), 'w', encoding='utf-8').write(txt)
