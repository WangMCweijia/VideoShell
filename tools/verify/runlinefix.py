#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""跑 LineFix.java：线路按钮 vs 分集 的离线断言（真实样本 + 合成样本）。"""
...
out = os.path.join(HERE, '_linefix')
os.makedirs(out, exist_ok=True)
r = subprocess.run([JAVAC, '-encoding', 'UTF-8', '-cp', cp, '-d', out,
                    os.path.join(HERE, 'LineFix.java')],
...
r = subprocess.run([JAVA, '-Dfile.encoding=UTF-8', '-Dsun.stdout.encoding=UTF-8',
                    '-cp', out + os.pathsep + cp, 'LineFix', _cp.samples('_s5')] + sys.argv[1:],
                   capture_output=True, text=True, encoding='utf-8', errors='replace')
txt = (r.stdout or '') + (('\n[stderr]\n' + r.stderr) if r.stderr else '')
sys.stdout.write(txt)
open(os.path.join(HERE, '_linefix.out.txt'), 'w', encoding='utf-8').write(txt)
