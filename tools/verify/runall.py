...
SUITES = [
    ...
    'runhlsfetch',
    'runokcookie',
    # v1.0.67：聚合搜索左栏"随结果长出来"（没结果的站不上栏）。真跑 RailGrowth 的
    # 三个决策，判据与 AggSearch.block 同源（D 段直接对照）—— 防"上栏看一套、
    # 网格看另一套"那种只在界面上才看得见的不一致。
    'runrailgrowth',
    # v1.0.67：一个站配一串网址、运行时并发赛马挑最快的。**真跑调度**（探针注入）：
    # "最快者胜"与"不等最慢的"是按**耗时**断言的（写成 awaitAll 的实现必挂 C2）。
    # G 段是源码接线守卫 —— 那些改动编译也过、只是功能静默失效。
    'runmirrorrace',
    # v1.0.68：站名净化（E49）。纯函数、无网络无样本 —— 牢骚 title（「跑路了」）
    # 必须剔空/回落 host，正常站名必须逐字节保留（误伤为零）。
    'runsitename',
    # v1.0.69：网盘媒体凭据合成（E50「直链 403」）。纯函数、无网络 ——
