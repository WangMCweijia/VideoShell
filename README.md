...
| 调试校准 | **站点页与影片详情页标题栏都有「校准」**：自动适配不灵时，用带指引的网页依次点**分类名 → 影片 → 分集**，每步从真实点击里抽出这类链接的**形状**（`/bspvt/{slug}.html` 等），最后真解析一次并试播，成功即固化进「站点配方」—— 之后任何页面新建适配器都直接按它走。判错会当场说明为什么（"这看着不是分集链接"），站点改版导致规则失效时会自动退回默认逻辑、不会卡死。见下文「调试校准模式」 |
...
`runbs4`（**分集名 + 校准逻辑 43 条**：剧名前缀剥离、分类形状、容器反推、配方优先级与失效自愈）。
...
**入口**：站点页右上角「校准」。整个模式是一个带指引的网页，三步：

| 步骤 | 提示 | 学到什么（推不出来会明确告诉你为什么） |
|---|---|---|
| ① | 点一个**分类名** | 分类页 URL 形状 `/bspvt/{slug}.html` + 所在导航容器 |
| ② | 点一部**影片** | 详情页模板 `/bspvd/{id}.html` |
| ③ | 点一集**分集** | 播放页模板 `/bspvp/{id}-4-1.html`，然后**真解析一次并试播** |
...
5. **分类认不出 / 详情打不开 / 分集名不对时，点右上角的「校准」**（站点页和影片详情页都有）：
   带指引地依次点分类名 → 影片 → 分集，规则会固化到本地，之后进这个站就直接按校准结果走
   （也可以随时「重来」重新校准）。站点页的「自检」报告末尾会列出当前配方，能看出它是自动学的还是校准固化的。
