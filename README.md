...
| 站点自检 | 站点页标题栏「自检」：把整条链路在当前网络下重跑一遍 —— 首页 → 分类 → 列表 → 详情 → 选集 → 播放地址 → **媒体请求** → **首个分片**，每步都带 **HTTP 状态码**，并附完整请求记录，可一键复制。**报告头带 App 版本号**，免得"装的到底是哪个包"来回扯。定位"为什么这个站用不了"不用再靠猜。**最后一步「首个分片」是关键**：playlist 拿 200 不代表分片也能下（分片常被单独做防盗链或落在另一个 CDN），这正是"能解析却放不出来"的唯一盲区 |
