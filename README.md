...
| 调试校准 | 站点页标题栏「校准」：自动适配不灵时，用带指引的网页依次点**分类名 → 影片 → 分集**，每步从真实点击里抽出这类链接的**形状**（`/bspvt/{slug}.html` 等），最后真解析一次并试播，成功即固化进「站点配方」—— 之后任何页面新建适配器都直接按它走。判错会当场说明为什么（"这看着不是分集链接"），站点改版导致规则失效时会自动退回默认逻辑、不会卡死。见下文「调试校准模式」 |
...
5. **分类认不出 / 详情打不开时点站点页右上角的「校准」**：带指引地依次点分类名 → 影片 → 分集，
   规则会固化到本地，之后进这个站就直接按校准结果走（也可以随时「重来」重新校准）。
   站点页的「自检」报告末尾会列出当前配方，能看出它是自动学的还是校准固化的。
