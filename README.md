...
| 网页嗅探 | WebView 钩住 XHR/fetch/video 事件 + `shouldInterceptRequest` 拦截媒体请求，按 HLS>DASH>MP4>FLV 打分排序，捕获到 HLS 自动跳转播放。**主文档加载失败或长时间抓不到地址时，状态栏直接说明原因**（错误码 / HTTP 码 / "可能需登录或手动点播放按钮"），不再让人对着空白页猜 |
